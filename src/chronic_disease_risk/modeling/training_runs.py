...
from chronic_disease_risk.modeling.time_split import split_train_test_by_time
from chronic_disease_risk.modeling.tuning import tune_xgboost_pipeline


def _artifact_label_suffix(artifact_label: str | None) -> str:
    return "" if not artifact_label else f"_{artifact_label}"
...
def _train_test_data(
    ...
    *,
    ...
    if split_strategy == "time":
        X_train, X_test, y_train, y_test, _ = split_train_test_by_time(
            df,
...
def run_baseline_training(
    ...
    *,
    ...
    X_train, X_test, y_train, y_test = _train_test_data(
        dataset_path,
        target_column,
        feature_columns,
        random_state,
    ...
    suffix = _artifact_label_suffix(artifact_label)

    metrics_path = output_dir / f"baseline{suffix}_{target_column}_metrics.csv"
    model_path = output_dir / f"baseline{suffix}_{target_column}.joblib"

    roc_plot_path = output_dir / f"baseline{suffix}_{target_column}_roc.png"
    confusion_matrix_path = output_dir / f"baseline{suffix}_{target_column}_confusion.png"

    calibration_plot_path = output_dir / f"baseline{suffix}_{target_column}_calibration.png"
    dca_plot_path = output_dir / f"baseline{suffix}_{target_column}_dca.png"

    write_metrics_table(metrics, metrics_path)
    save_roc_curve_plot(y_test.to_numpy(), y_prob, roc_plot_path, title=f"Baseline ROC - {target_column}{suffix}")
    save_confusion_matrix_plot(
        y_test.to_numpy(),
        y_pred,
        confusion_matrix_path,
        title=f"Baseline Confusion - {target_column}{suffix}",
    )
    save_calibration_curve_plot(
        y_test.to_numpy(),
        y_prob,
        calibration_plot_path,
        title=f"Baseline Calibration - {target_column}{suffix}",
    )
    save_decision_curve_plot(y_test.to_numpy(), y_prob, dca_plot_path, title=f"Baseline DCA - {target_column}{suffix}")
    ...
    *,
    ...
    if enable_tuning and "xgboost" in candidates:
        ...
        if xgb_pipeline is not None:
            tuned_path = output_dir / f"candidate{suffix}_{target_column}_xgboost_tuned_params.json"
    ...
    scores = {
        row["model_name"]: {k: float(row[k]) for k in ["auc", "accuracy", "precision", "recall", "f1"]}
        for row in rows
    }
    ...
    summary_path = output_dir / f"candidate{suffix}_{target_column}_summary.csv"
    model_path = output_dir / f"candidate_best{suffix}_{target_column}.joblib"
    comparison_plot_path = output_dir / f"candidate{suffix}_{target_column}_comparison.png"

    best_roc_plot_path = output_dir / f"candidate_best{suffix}_{target_column}_roc.png"
    best_confusion_matrix_path = output_dir / f"candidate_best{suffix}_{target_column}_confusion.png"
    best_calibration_plot_path = output_dir / f"candidate_best{suffix}_{target_column}_calibration.png"
    best_dca_plot_path = output_dir / f"candidate_best{suffix}_{target_column}_dca.png"
    ...
    save_model_comparison_plot(
        ...
        metric_name="auc",
        title=f"Candidate Comparison - {target_column}{suffix}",
    )
    ...
    save_confusion_matrix_plot(
        ...
        title=f"Candidate Best Confusion - {target_column}{suffix}",
    )
    save_calibration_curve_plot(
        y_test.to_numpy(),
        y_prob_best,
        best_calibration_plot_path,
        title=f"Candidate Best Calibration - {target_column}{suffix}",
    )
    save_decision_curve_plot(
        y_test.to_numpy(),
        y_prob_best,
        best_dca_plot_path,
        title=f"Candidate Best DCA - {target_column}{suffix}",
    )
...
def run_all_baseline_trainings(
    ...
    *,
    ...
    return {
        task_name: run_baseline_training(
